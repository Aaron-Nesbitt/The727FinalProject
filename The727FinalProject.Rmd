...
```{r}
freq_terms_apple2_less <- frequent_terms_apple2[-c(1,2,3,4,5,6,8),]
```
...
#The function rowSums computes column sums across rows for each level of a grouping variable.
plotemot_apple <- data.frame(rowSums(emotionsALLt_apple[1:17412]))
#Transformation and cleaning
names(plotemot_apple)[1] <- "count"
plotemot_apple <- cbind("sentiment" = rownames(plotemot_apple), plotemot_apple)
rownames(plotemot_apple) <- NULL
plotemot_apple2<-plotemot_apple[1:8,]
#Plot One - count of words associated with each sentiment
quickplot(sentiment, data=plotemot_apple2, weight=count, geom="bar", fill=sentiment, ylab="count")+ggtitle("Survey sentiments")
...
#The function rowSums computes column sums across rows for each level of a grouping variable.
plotemot_covid <- data.frame(rowSums(emotionsALLt_covid[1:17412]))
#Transformation and cleaning
names(plotemot_covid)[1] <- "count"
plotemot_covid <- cbind("sentiment" = rownames(plotemot_covid), plotemot_covid)
rownames(plotemot_covid) <- NULL
plotemot_covid2<-plotemot_covid[1:8,]
#Plot One - count of words associated with each sentiment
quickplot(sentiment, data=plotemot_covid2, weight=count, geom="bar", fill=sentiment, ylab="count")+ggtitle("Survey sentiments")
